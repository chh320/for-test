    ...
    , _modelScale(1.0)
{
    ...
    CreateGraphicsPipeline(12, config.isWireFrame);
...
void DebugLodApplication::Run(std::vector<uint32_t>& packedData, uint32_t meshNum)
{
    CreateCamera();
    CreateCommandBuffer();
    ...
    RecordCommand(packedData, meshNum);
    SetEvents();
    DrawFrame();
...
void DebugLodApplication::RecordCommand(std::vector<uint32_t>& packedData, uint32_t meshNum)
{
    std::vector<uint32_t> graphicsPushConstants(3);
    ...
    for (auto i = 0; i < _commandBuffers->GetSize(); i++) {
        ...
        for (int j = 0; j < meshNum; j++) {
            graphicsPushConstants[2] = j;
            PushConstant(cmd, _graphicsPipeline->GetPipelineLayout(), 12, graphicsPushConstants.data());
            auto meshOffset = 0;
            for (int k = 0; k < j; k++) {
                meshOffset = packedData[3 + meshOffset];
            }
            auto instanceNum = packedData[meshOffset];
            Draw(cmd, instanceNum);
        }
...
void DebugLodApplication::Draw(VkCommandBuffer cmd, uint32_t instanceNum)
{
    if (_useInstance) {
        vkCmdDraw(cmd, 3 * 128, instanceNum, 0, 0);
    } else {
