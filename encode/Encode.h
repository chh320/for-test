...
class Encode final {
public:
    static void PackingMeshData(const std::string& modelFileName, const std::vector<VirtualMesh>& vmeshes, std::vector<uint32_t>& packedData)
    {
        Util::Timer timer;
        auto vmeshId = 0;
        for (auto vmesh : vmeshes) {
            ...
            packedData.push_back(0);                                    // next mesh offset
            ...
            auto meshOffset = 0;
            for (int i = 0; i < vmeshId; i++) {
                meshOffset = packedData[meshOffset + 3];
            }

            packedData[meshOffset + 2] = packedData.size();
            ...
            i = 0;
            for (auto& group : vmesh.GetClusterGroups()) {
                uint32_t offset = packedData[meshOffset + 2] + 8 * i;
                packedData[offset + 1] = packedData.size();
            ...
            packedData[meshOffset + 3] = packedData.size();
            vmeshId++;
